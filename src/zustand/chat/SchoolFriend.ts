...
const SchoolFriendStore = create<ChatState>((set) => ({
  links: null,
  count: 1,
  current: 2,
  newCount: 0,
  ...
  addFriendsChat: async (saved: ChatContent) => {
    SchoolFriendStore.setState((prev) => {
  ...
  getFriends: async (
    ...
    try {
      const response = await apiRequest<FetchChatResponse>(url, {
        setMessage,
        setLoading: SchoolFriendStore.getState().setLoading,
      })
      const data = response?.data
      if (data) {
        SchoolFriendStore.getState().setProcessedResults(data.results)
      }
    } catch (error: unknown) {
      console.log(error)
    }
...
export default SchoolFriendStore
