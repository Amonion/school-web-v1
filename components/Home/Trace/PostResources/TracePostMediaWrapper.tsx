...
interface TracePostMediaWrapperProps {
  sources: MediaSource[]
}

const TracePostMediaWrapper: React.FC<TracePostMediaWrapperProps> = ({
  sources,
}) => {
          ...
          {sources.length === 1 ? (
            ...
            <div className="flex gap-1">
              {sources.map((media, index) => (
                <div
                  onClick={() => setMedia(sources[index].source)}
                  key={index}
...
export default TracePostMediaWrapper
