...
import PostEditor from '@/components/Home/Editor/PostEditor'
import CreatePostMediaDisplay from '@/components/Home/Media/CreatePostMediaDisplay'
...
const CreatePost: React.FC = () => {
  ...
  return (
    <>
      <div className="w-full flex-col flex-1 pt-3 flex justify-end items-center">
        <div className="flex-1 w-full">
          {user && (
            <div className="flex mb-[6px] px-2 sm:pr-2 md:px-0 w-full">
              {
                <div className="w-10 h-10 rounded-full overflow-hidden mr-3 border border-[var(--border)]">
                  {user && (
                    <Image
              ...
              <div className="flex-1">
                <div className="flex items-start flex-wrap">
                  <div className="flex items-center mb-2">
                    <span className="account_name mr-2">
                      {user.displayName}
                    </span>
                    {user.isVerified && (
                ...
                <div className="flex justify-between">
                  <span className="post_username mt-[-8px]">
                    @{user.username}
                  </span>
          ...
          {!isPoster && (
            <div
              className="mb-3 px-2 sm:pr-2 md:px-0"
              dangerouslySetInnerHTML={{
                __html:
                  countText(text) > 0 ? text : 'Start your post content...',
              }}
            />
          )}
          {files.length > 0 && !isPoster && (
            <CreatePostMediaDisplay sources={files} />
          )}
          {isPoster && (
            <div
              style={{ backgroundColor: color }}
              className="relative w-full flex-1 text-white  flex justify-center items-center min-h-[200px]"
            >
              <div
        ...
        <div
          ...
          className="bg-[var(--primary)] mt-10 relative p-3 w-full"
        >
          {isColor && (
              ...
              <div key={index} className="w-20">
                <div className="relative w-20 h-20 overflow-hidden rounded-[5px] mb-[-10px]">
                  <button
                    ...
                    className="absolute top-1 right-1 z-20 bg-[var(--white-gray)] text-[var(--text-secondary)] rounded-full p-1 w-5 h-5 flex items-center justify-center text-xs"
                  >
                    ✕
                  </button>
