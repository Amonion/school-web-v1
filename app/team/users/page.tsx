...
const Users: React.FC = () => {
  ...
  const { bioUsers, getBioUsers } = BioUserStore()
  ...
  useEffect(() => {
    ...
    getBioUsers(`${url}bio-user/${params}`, setMessage)
  ...
  useEffect(() => {
    if (results.length === 5) {
      getItems(
        `/messages/?page_size=${emailPageSize}&page=${emailCurrentPage}&ordering=title`,
        setMessage
      )
    }
  ...
  const makeStaff = async (user: BioUser) => {
    const params = `?page_size=${page_size}&page=${currentPage}&ordering=${sort}&userStatus=User`
    await updateUser(`${url}${user._id}${params}`, {}, setMessage)
  }
  ...
  return (
    <>
      <div className="card_body">
        <div className="custom_sm_title">Table of Users</div>
