...
const StudentApplicants: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (applicants.length === 0) {
      getApplicants(
        `/offices/?username=${officeForm.username}&isUserApplied=true&page_size=${page_size}&page=${currentPage}&userType=Student`,
        setMessage
      )
    }
    getItems(
  ...
  const approveVerification = (approval: boolean) => {
    ...
    updateApplicants(
      `/schools/approve-application/${officeForm.username}?isUserApplied=true&page_size=${page_size}&page=${currentPage}&username=${officeForm.username}&userType=Student`,
      data,
      setMessage,
      () => setDisplayBox(false)
    )
  ...
  return (
    <>
      <PageTitle page="Student Applicants" title={officeForm.name} />
