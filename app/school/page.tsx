  ...
  return (
    <>
      {bioUserState?.activeOffice.position !== 'Unknown' ? (
        <div className="space-y-2  text-[var(--text-primary)] w-full">
          {bioUserState && bioUserState.activeOffice && (
            <PageTitle
              page={bioUserState?.activeOffice?.position}
              title={bioUserState?.activeOffice?.name}
            />
          )}
